...
do
    ...
    --learning_rate=${lr} | tee -a ./Vcvpr_abalation_ongap_resnet18_binary_reparam_init_fpw${fpw}_resl${resl}_${opt_type}_w_${wd}_seed${s}_${data}_target_${target}_varw_rate${varw[k]}_lr${lr}_type${nettype}.txt&&
    echo "done $target" &
done
wait
done
