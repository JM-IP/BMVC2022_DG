...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, binary=True, get_binary_feat=False, distortion=False):
        ...
        res=0
        if binary:
            out1 = self.binary_3x3(out1, binary)
        else:
            out1, restmp = self.binary_3x3(out1, binary, distortion=distortion)
            res+=restmp
        out1 = self.bn1(out1)
        ...
        if self.inplanes == self.planes:
            ...
            if binary:
                out2 = self.binary_pw(out2, binary, distortion=distortion)
            else:
                out2, restmp = self.binary_pw(out2, binary, distortion=distortion)
                res+=restmp
            out2 = self.bn2(out2)
            out2 += out1
            ...
            if binary:
                out2_1 = self.binary_pw_down1(out2, distortion=distortion)
                out2_2 = self.binary_pw_down2(out2, distortion=distortion)
            else:
                out2_1, restmp = self.binary_pw_down1(out2, binary, distortion=distortion)
                res += restmp
                out2_2, restmp = self.binary_pw_down2(out2, binary, distortion=distortion)
                res += restmp
...
class ReActNet(nn.Module):
    ...
    def forward(self, x, gt=None, binary=True, flag=None, epoch=None, distortion=False):
        ...
        for i, block in enumerate(self.feature):
            ...
            if i==0:
                ...
                out = block(x, binary=binary, get_binary_feat=cal_var_loss, distortion=distortion)
                x = out['out']
                if cal_var_loss:
                    binfeat = out['feat']
                    feat += binfeat
    ...
    if pretrained:
        ...
        for k, v in a.items():
            ...
            if 'fc' in name:
                ...
                # print(v.size(), model_state[name].size())
