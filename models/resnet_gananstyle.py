...
class ResNet(nn.Module):
    def __init__(self, block, layers, jigsaw_classes=1000, classes=100, with_style_adain=False, p_adain=None):
        self.inplanes = 64
    ...
    def forward(self, x, gt=None, flag=None, epoch=None, noise=None, randyes=False, type='feat+image'):
        ...
        if ganstyle and self.training and 'image' in type:
            ...
            image_out_fake_feat = [image_out_fake]
            for layers in [self.layer1, self.layer2, self.layer3, self.layer4]:
                for layer in layers:
        ...
        if ganstyle and self.training:
            ...
            if 'image' in type:
                out.update({"image_out_loss_c": loss_c, "image_out_loss_s": loss_s})
            return out
        return x
