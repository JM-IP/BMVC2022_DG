...
class BiRealNet(nn.Module):
    ...
    def forward(self, x, gt=None, flag=None, epoch=None, more_out=False):
        ...
        for layers in [self.layer1, self.layer2, self.layer3, self.layer4]:
            for layer in layers:
                if cal_var_loss:
                    x, binfeat = layer(x, cal_var_loss)
                    out_all.append(binfeat)
                else:
                    x = layer(x, cal_var_loss)
        if cal_var_loss:
            if self.var_type=='last':
                self.var_mean[0].update(out_all[-1].mean([0,2,3]))
                tmp = out_all[-1].var([2,3]).pow(2).mean(0)
                self.var_std[0].update(tmp)
                var_loss = tmp.mean()
                mean_loss = out_all[-1].abs().mean()
            elif self.var_type=='all':
                var_loss = 0
                mean_loss = 0
                var_std = []
                for num, i in enumerate(out_all):
...
def birealnet18(pretrained=True, classes=7, **kwargs):
    """Constructs a BiRealNet-18 model. """
    model = BiRealNet(BasicBlock, [4, 4, 4, 4], num_classes=classes, **kwargs)
    print("hi")
    if pretrained=='./models/model_best_resnet_oldv.pth.tar':
        a = torch.load('./models/model_best_resnet_oldv.pth.tar')['state_dict']
