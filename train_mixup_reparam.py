...
class Trainer:
    ...
    def _do_epoch(self, epoch=None):
        ...
        for it, ((data, jig_l, class_l), d_idx) in enumerate(self.source_loader):
            ...
            if self.args.two_step:
                self.model.eval()
                out = self.model.forward(x=data_shuffle, gt=class_l_shuffle, binary=False, distortion=True, flag=self.flag, epoch=epoch)
    ...
    def do_test(self, loader):
        ...
        final_out = {"class_correct": class_correct, "class_correct_fp": class_correct_fp, 'class_loss': class_loss}
        if self.args.cal_var_loss:
